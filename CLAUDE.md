...
- `//` で始まるメッセージは読み上げない（URL要約・画像概要も行わない）。全角 `／` および半角と全角の混在も対象で、先頭の空白は無視する
...
- リプライの場合は「{ユーザー名}、{リプライ先}へのリプライ、{本文}」形式（ユーザー名を省略する場合もリプライ先は読み上げる）
  - 自分自身への返信はリプライ先を読み上げない（冗長なため）
  - リプライ先はメッセージに含まれる返信先情報から取得する（追加の API 呼び出しなし）。取得できない場合や名前が空になる場合はリプライ表記を省く
- ユーザー名: ニックネーム優先 → 表示名にフォールバック（両方空なら名前なし）
- ユーザーメンション（`<@id>`）・ロールメンション（`<@&id>`）を `@名前` に解決してから本文処理（解決できないメンションは従来通り削除）
- スポイラー（`||...||`）を含むメッセージ、およびスポイラー添付（`SPOILER_` プレフィックス）があるメッセージは本文全体を「ネタバレ防止」に置き換える（一部だけ読んでも前後の文脈からネタバレしうるため）。ユーザー名・リプライ先は読み上げ、添付の枚数は読み上げない。定型文なので辞書は適用しない。コードブロック内の `||` は対象外
  - スポイラー添付は画像・動画以外（PDF など）も対象
...
- スポイラー画像は概要を取得しない（伏せられた内容をチャットに書き出さないため）
